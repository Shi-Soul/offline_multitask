...
class ClippedDistribution(Distribution):
    def __init__(self, base_distribution, min_val=-1, max_val=1):
        super().__init__()
    ...
    def sample(self, sample_shape=torch.Size()):
        return torch.clamp(self.base_distribution.sample(sample_shape), self.min_val, self.max_val)
    
    def log_prob(self, value):
        # FIXME: assume outlier have 0 probability
        # if value is in the critical point, it will output a base log prob
        # but in fact it have a very large prob
        # as a result, the integral of this distribution is not 1
        in_range = (value >= self.min_val) & (value <= self.max_val)
        in_range = torch.all(in_range, dim=-1)
        base_log_prob = self.base_distribution.log_prob(value)
        return torch.where(in_range, base_log_prob, float('-inf'))
    
    def entropy(self):
        # FIXME: it's wrong
        # but even for gaussian distribution, we don't have a closed form solution
...
def test_gail(args=get_args()):
    ...
    train_envs = SubprocVectorEnv(
        [lambda: NoRewardEnv(get_gym_env(args.task)) for _ in range(args.training_num)],
        # norm_obs=True
    )
    ...
    def dist(*logits):
        return ClippedDistribution(Independent(Normal(*logits), 1))
    ...
    expert_buffer = load_buffer_dataset()
