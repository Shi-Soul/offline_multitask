...
class FlattenJacoObservationWrapper(dm_env.Environment):
    def __init__(self, env):
        ...
        wrapped_obs_spec = env.observation_spec().copy()     # action space dimensions
...
class ActionRepeatWrapper(dm_env.Environment):
    ...
    def step(self, action):
        reward = 0.0
        discount = 1.0
        for i in range(self._num_repeats):     # multi-steps
            time_step = self._env.step(action)
            reward += time_step.reward * discount
            discount *= time_step.discount     # use discount
            if time_step.last():
                break
...
def _make_dmc(obs_type, domain, task, frame_stack, action_repeat, seed):
    ...
    # if: the task are DMC standard task, then use suite.load
    # else: the task is new task defined in cdmc, then use cdmc.make
